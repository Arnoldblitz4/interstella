...
export type Spawn = { t:number; x:number; kind:"scout"|"zig"|"tank"|"swarmer"|"elite"|"shielded" };
...
export function generateLevel(id:number):GeneratedLevel {
  ...
  const kind:LevelKind=safeId===30||safeId%11===0?"boss":safeId%3===1?"survival":safeId%3===2?"escort":"liberation";
  ...
  for(let i=0;i<count;i++) {
    const roll=r();
    const kind2=roll>.94?"tank":roll>.84?"shielded":roll>.76?"elite":roll>.58?"zig":roll>.34?"swarmer":"scout";
